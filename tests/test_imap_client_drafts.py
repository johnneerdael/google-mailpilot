...
class TestImapClientDrafts:
    ...
    def test_get_drafts_folder_gmail(self, imap_client):
        ...
        imap_client.config.host = "imap.gmail.com"

        # Mock list_folders to include Gmail's standard folders
    ...
    def test_get_drafts_folder_fallback(self, imap_client):
        ...
        # Should return INBOX as fallback
    ...
    def test_save_draft_mime_success(self, imap_client, sample_mime_message):
        ...
        # imapclient.append signature is append(folder, msg, flags=None, msg_time=None)
        # It's usually passed as a keyword argument 'flags' by imapclient internally if we pass it to its append
        # But here we are calling imap_client.client.append directly or via wrapper
        # Let's check how it's actually called in imap_client.py
        assert "\\Draft" in kwargs.get("flags", [])
    ...
    def test_save_draft_mime_failure(self, imap_client, sample_mime_message):
        ...
        imap_client.client.append.side_effect = Exception("IMAP error")
