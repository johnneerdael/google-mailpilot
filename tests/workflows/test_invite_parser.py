...
class TestInviteParser:
    ...
    def test_fallback_to_email_date(self):
        ...
        if result["is_invite"]:
            ...
            # Should fall back to email date (returned as ISO string)
            start_time = datetime.fromisoformat(result["details"]["start_time"])
            assert start_time.date() == datetime(2025, 4, 5).date()
